...
bool FlashStorage::load(CalibrationState& out_state) {
    ...
    if (valid) {
        ...
        for (int i = 0; i < CAL_FORCE_LEVEL_COUNT; i++) {
            out_state.cw_speed[i].store(cal_data.cw_speed[i]);
            out_state.ccw_speed[i].store(cal_data.ccw_speed[i]);
        }
        out_state.system_damper_strength.store(cal_data.system_damper_strength);
        out_state.forward_max_pwm.store(cal_data.forward_max_pwm);
        ...
        out_state.forward_max_pwm.store(DEFAULT_FORWARD_MAX_PWM);
...
bool FlashStorage::save(const CalibrationState& state, bool core1_running) {
    ...
    for (int i = 0; i < CAL_FORCE_LEVEL_COUNT; i++) {
        data.cw_speed[i] = state.cw_speed[i].load();
        data.ccw_speed[i] = state.ccw_speed[i].load();
    }
    data.wheel_angle_deg = state.wheel_angle_deg.load();
    data.system_damper_strength = state.system_damper_strength.load();
    data.forward_max_pwm = state.forward_max_pwm.load();
